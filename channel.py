...
from utils import Game, invalidate_cache, json_minify
...
class Channel:
    ...
    async def get_stream(self) -> Stream | None:
        try:
            response: JsonType = await self._twitch.gql_request(
                GQL_OPERATIONS["GetStreamInfo"].with_variables(
                    {"channel": self._login},
                ),
            )
        ...
        if not stream.drops_enabled:
            try:
                ...
                stream.drops_enabled = any(
                    ...
                    for c in (
                        available_drops["data"]["channel"]["viewerDropCampaigns"] or []
                    )
                )
        return stream
    ...
    async def _online_delay(self) -> None:
        ...
        await self.update_stream(
            trigger_events=True,
        )  # triggers 'display' via the event
    ...
    async def claim_bonus(self) -> None:
    ...
    async def send_watch(self) -> bool:
        ...
        try:
            response: JsonType = (
                await self._twitch.gql_request(  # Gets signature and value
            ...
            msg = f"Channel: {self._login}"
            raise MinerException(msg) from exc
        signature: JsonType | None = response["data"]["streamPlaybackAccessToken"][
            "signature"
        ]
        ...
        BroadcastLowestQualityURL = BroadcastQualities.split(
            ...
            -1
        ]  # Just takes the last line, this could probably be handled better in the future
        ...
        StreamLowestQualityURL = StreamURLList.split(
            ...
            -2
        ]  # For whatever reason this includes a blank line at the end, this should probably be handled better in the future
        if not validators.url(StreamLowestQualityURL):
            return False

        try:
            async with (
                self._twitch.request(  # Downloads the stream
                    "HEAD",
                    StreamLowestQualityURL,
                ) as response3
            ):  # I lied, well idk, but this code doesn't listen for the actual video data
